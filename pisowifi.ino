...
void loop() {
   ...
   if (coins < 1 && gate == 0 && start == 0) {
    //if (client.connected()) {
      ...
      print_txt("                    ", 0, 2);
      print_txt("1=12m 20=6hr 30=10hrs", 0, 3);
...
//    }
  ...
  if ((coins == 0) && currentMillis - startMillis >= 5000) {
     coinInterrupt();
  }
  
  if ((start == 1) && (currentMillis - startMillis >= 100)) {    
...
//    Serial.println(currentMillis - startMillis >= 5000);
    ...
    if ((coins > 0) && currentMillis - startMillis >= 5000) {
      ...
      Serial.println("To Generate Code");
      Serial.println("");
